#!/usr/bin/env python3
"""
双标的 第二轮深探 — 在第一轮最优基础上继续探索
=================================================
第一轮已固定:
  HLDB G1_BUY=G2_BUY=1%, HLDB SHARES=900
  HST G1_BUY/G2_BUY 独立搜索已完成

本轮新探索:
  阶段 A: 固定第一轮最优 HST，扫描 HLDB 买入对 (G1_BUY, G2_BUY) + HLDB 股数
  阶段 B: 固定最优 HLDB 买入+股数，重扫 HST (买入对 × 卖出 × 股数)，验证交互效应
  阶段 C: 全参固定，细粒度预算扫描 (200元步长)

读取第一轮结果: dual_sweep_best.txt (解析最优参数) 或使用硬编码 fallback
输出: dual_sweep2_full.csv, dual_sweep2_best.txt
"""
...
def load_best_from_txt(path="dual_sweep_best.txt"):
    ...
    # HST 网格 (支持 G1_BUY / G2_BUY 或旧版 BUY=)
...
# HLDB 买入对 (G1_BUY, G2_BUY)，要求 G2_BUY >= G1_BUY
...
# 细粒度预算 (200元步长)
BUDGET_GRID = list(range(2000, 4801, 200))   # 2000~4800
...
def stage_a_hldb_buy(merged_df, p0):
    """阶段 A: 固定第一轮最优 HST，扫描 HLDB 买入对 × HLDB 股数。"""
    print("\n━━━ 阶段 A: HLDB 买入对 × 股数 扫描 ━━━")
    ...
    results = []
    t0 = time.time()
    for i, ((hb1, hb2), hsh) in enumerate(combos):
        r = patch_and_run(
            merged_df,
        ...
        if (i + 1) % 10 == 0 or (i + 1) == len(combos):
            ...
            print(f"  [{i+1}/{len(combos)}] elapsed={elapsed:.1f}s  eta={eta:.1f}s", end="\r")
    print()
...
def stage_b_hst_reverify(merged_df, bestA):
    """阶段 B: 固定最优 HLDB（含买入），重扫 HST 全参数，验证交互效应。"""
    print("\n━━━ 阶段 B: HST 全参数重扫（验证交互效应）━━━")
    combos = list(itertools.product(HST_BUY_PAIRS, HST_SELL_PAIRS, HST_SHARES_GRID))
    print(f"  共 {len(combos)} 组")
    results = []
    t0 = time.time()
    for i, ((hb1, hb2), (hs1, hs2), hssh) in enumerate(combos):
        r = patch_and_run(
            merged_df,
...
def main():
    print("=" * 60)
    print("双标的参数深探 Round 2 — HLDB买入/股数 + 交互验证 + 细预算")
    print("=" * 60)
    ...
    # ── 阶段 A ─────────────────────────────────────────────
    dfA, bestA = stage_a_hldb_buy(merged_df, p0)

    # ── 阶段 B ─────────────────────────────────────────────
    dfB, bestB = stage_b_hst_reverify(merged_df, bestA)
