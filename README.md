...
```bash
python backtest.py \
  --portfolio "515450.SH:0.50,513130.SH:0.30,510300.SH:0.20" \
  --monthly 6000 \
  --rebalance-months 6
```

权重会自动归一化，所以 `50,30,20` 与 `0.5,0.3,0.2` 等价。

也可以写不带市场后缀的沪深代码；程序会自动补 `.SH` / `.SZ`：

```bash
python backtest.py --portfolio "515450:80,513130:20"
```
...
```bash
python backtest.py \
  --portfolio "515450.SH:0.8,513130.SH:0.2" \
  --lot-sizes "515450.SH:100,513130.SH:100"
```
